...
### 6. Turning `fleet_size` down never actually shrank the dashboard

**Symptom:** found live, on the real Render deployment: ramp `fleet_size` up (e.g. for a scale demo) and then back down, and the dashboard keeps showing the old, larger count indefinitely — "All" and "Needs attention" never shrink back down. `GET /metrics`'s `state_map_size` confirmed this wasn't a display bug: the backend really was still holding every robot it had ever seen.

**Root cause:** this was the expected result of the "No separate remove robot admin action" design tradeoff documented below — a descaled robot only ever gets flagged `offline` by the staleness sweep. `evictRobot` (`ingest.ts`), the one function that would actually remove a robot from the state map, existed but was never called from anywhere.

**Fix:** extended `sweepStaleness` (`state.ts`) to take an optional second threshold, `evictAfterMs`: a robot already marked `offline` is now removed from state once it's stayed silent that much longer — a separate, later step from the moment it first goes `offline`, not a replacement for it. The original guarantee holds exactly as before (a robot never vanishes the instant it goes quiet; it still becomes visibly `offline` first) — it just no longer sits in memory forever after that. Two new tests cover both the eviction and the "not yet" case (`tests/state.test.ts`); every pre-existing test passes unchanged, since the new parameter defaults to "never evict" when a caller doesn't pass it.
...
- **No separate "remove robot" admin action.** When `fleet_size` is turned down, the simulator just stops those agents; the backend has no explicit deletion path, so a descaled robot goes through the exact same staleness → `offline` path as a dropped connection. One mechanism covers both cases, and it matches the PRD's own framing that a robot going silent should never make it silently vanish. (Update, see Finding #6: that path used to end at `offline` forever, which meant the dashboard's counts never actually shrank after a scale-down — fixed by adding a later, separate eviction step, without changing this core guarantee.)
